...
export function AddNewDialog({ triggerState, setTriggerState }: AddNewDialogProps) {
  const [isOpen, setIsOpen] = useState(false)
  const [showLayoutModal, setShowLayoutModal] = useState(false)
  const [selectedLayout, setSelectedLayout] = useState<BlogLayout>("image-scattered")
  ...
  const paragraphFileInputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({})
  ...
  const handleParagraphFileUpload = useCallback((files: FileList | null, paragraphId: string) => {
    if (!files || files.length === 0) return
    ...
    reader.onload = (e) => {
      ...
      setParagraphs((prev) => prev.map((p) => (p.id === paragraphId ? { ...p, image: newImage } : p)))
    }
    reader.readAsDataURL(file)
  ...
  const handleRemoveParagraphImage = (paragraphId: string) => {
    setParagraphs((prev) => prev.map((p) => (p.id === paragraphId ? { ...p, image: undefined } : p)))
  }
      ...
      prev.map((p) =>
        p.id === paragraphId
          ? {
              ...
              formatting: {
                ...p.formatting,
                [format]: value !== undefined ? value : !p.formatting[format as keyof typeof p.formatting],
              },
            }
          : p,
      ),
  ...
  const setAlignment = (paragraphId: string, align: "left" | "center" | "right") => {
    setParagraphs((prev) =>
      prev.map((p) => (p.id === paragraphId ? { ...p, formatting: { ...p.formatting, align } } : p)),
    )
  }
  ...
  const ColorPicker = ({ paragraphId, currentColor }: { paragraphId: string; currentColor: string }) => (
    <Select value={currentColor} onValueChange={(value) => applyFormatting(paragraphId, "color", value)}>
      <SelectTrigger className="w-20 h-8">
        <div className="flex items-center gap-1">
          <Palette className="h-3 w-3" />
          <div className="w-3 h-3 rounded" style={{ backgroundColor: currentColor }} />
        </div>
      </SelectTrigger>
      <SelectContent>
  ...
  const FontSizeSelector = ({ paragraphId, currentSize }: { paragraphId: string; currentSize: string }) => (
    <Select value={currentSize} onValueChange={(value) => applyFormatting(paragraphId, "fontSize", value)}>
  ...
  const FontFamilySelector = ({ paragraphId, currentFont }: { paragraphId: string; currentFont: string }) => (
    <Select value={currentFont} onValueChange={(value) => applyFormatting(paragraphId, "fontFamily", value)}>
      <SelectTrigger className="w-32 h-8">
        <div className="flex items-center gap-1">
          <Type className="h-3 w-3" />
          <span className="truncate">{fontFamilyOptions.find((f) => f.value === currentFont)?.label}</span>
        </div>
      </SelectTrigger>
      <SelectContent>
  ...
  const RichTextToolbar = ({
    paragraphId,
    formatting,
  }: { paragraphId: string; formatting: ParagraphSection["formatting"] }) => (
    <div className="flex items-center gap-2 p-2 border-b bg-muted/30 flex-wrap">
      <ToolbarButton icon={Bold} active={formatting.bold} onClick={() => applyFormatting(paragraphId, "bold")} />
      <ToolbarButton icon={Italic} active={formatting.italic} onClick={() => applyFormatting(paragraphId, "italic")} />
      <ToolbarButton
        icon={Underline}
      ...
      <FontSizeSelector paragraphId={paragraphId} currentSize={formatting.fontSize} />
      <FontFamilySelector paragraphId={paragraphId} currentFont={formatting.fontFamily} />
    </div>
  )
    ...
    >
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <span className="text-blue-500 hover:underline cursor-pointer">Browse</span>
        </div>
        <div className="text-xs text-muted-foreground">Maximum file size: 50mb, maximum 5 images</div>
      </div>
    </div>
  )

  const ParagraphImageUploadArea = ({ paragraphId }: { paragraphId: string }) => (
    <div
    ...
    >
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="text-xs text-muted-foreground">
          Add image (optional) - <span className="text-blue-500 hover:underline cursor-pointer">Browse</span>
        </div>
      </div>
    </div>
  )
        ...
        <div className="flex flex-col h-screen w-screen">
          ...
          <div className="flex items-center justify-between p-6 border-b bg-white shrink-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Manage your blogs here</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button className="bg-green-600 hover:bg-green-700" onClick={handlePost}>
                Post
              </Button>
            </div>
          </div>
            ...
            <div className="w-1/3 border-r bg-gray-50 flex flex-col">
              <div className="p-6 flex-1 overflow-y-auto">
                <div className="space-y-4">
                  ...
                  {heroImages.length > 0 && (
                    <div>
                      <h3 className="text-sm font-medium mb-2">Images Selected ({heroImages.length}/5)</h3>
                  ...
                  <div className="space-y-6 max-w-4xl">
                    ...
                    <div className="space-y-6">
                      ...
                      {paragraphs.map((paragraph, index) => (
                        <div key={paragraph.id} className="space-y-4 border rounded-lg p-4">
                          <div className="flex items-center justify-between">
                            <h3 className="text-sm font-medium">Paragraph {index + 1}</h3>
                            {paragraphs.length > 1 && (
                              <Button
                                variant="ghost"
                                size="sm"
                          ...
                          <div className="border rounded-lg overflow-hidden">
                            <RichTextToolbar paragraphId={paragraph.id} formatting={paragraph.formatting} />
                            <textarea
                              ...
                              onChange={(e) => {
                                setParagraphs((prev) =>
                                  prev.map((p) => (p.id === paragraph.id ? { ...p, content: e.target.value } : p)),
                                )
                                if (validationErrors.paragraphs) {
                                  setValidationErrors((prev) => ({ ...prev, paragraphs: undefined }))
                                }
                              }}
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label className="text-sm text-muted-foreground">Paragraph Image (Optional)</Label>
                            {paragraph.image ? (
                              <div className="relative">
                                <img
                                  src={paragraph.image.preview || "/placeholder.svg"}
                                ...
                                <Button
                                  variant="destructive"
                                  size="sm"
                                  onClick={() => handleRemoveParagraphImage(paragraph.id)}
                              ...
                              <ParagraphImageUploadArea paragraphId={paragraph.id} />
                            )}

                            <input
                              ref={(el) => {
                                if (el) paragraphFileInputRefs.current[paragraph.id] = el
                              }}
                              type="file"
                              accept="image/*"
                              className="hidden"
                              onChange={(e) => handleParagraphFileUpload(e.target.files, paragraph.id)}
                            />
                          </div>
                        </div>
                      ))}
                      ...
                      <div className="pb-8">
                        <Button onClick={addParagraph} className="bg-green-600 hover:bg-green-700">
                          <Plus className="w-4 h-4 mr-2" />
                          Add a paragraph
                        </Button>
                      </div>
